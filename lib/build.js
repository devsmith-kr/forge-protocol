...
export async function runBuild(options = {}) {
  ...
  const targetBlocks = targetBlockIds.map((id) => blockMap.get(id) ?? blocks.find((b) => b.id === id)).filter(Boolean);
  ...
  await writeFile(join(forgeDir, 'project', 'build-prompt.md'), prompt, 'utf-8');
  ...
  if (options.ai) {
    await runAiPrompt({
      promptText: prompt,
      outputPath: join(forgeDir, 'project', 'build-ai-response.md'),
      phaseName: 'Forge — 코드 생성',
      model: options.model,
    });
    console.log(chalk.bold('  다음 단계:'));
    console.log(
      chalk.dim('    1. ') +
        chalk.white('build-ai-response.md') +
        chalk.dim(' 의 생성된 코드를 검토하세요.'),
    );
    console.log(
      chalk.dim('    2. 코드를 ') + chalk.white('.forge/generated/src/') + chalk.dim('에 저장하세요.'),
    );
    console.log(chalk.dim('    3. ') + chalk.cyan('forge temper') + chalk.dim(' — 테스트 시나리오 생성'));
  } else {
    console.log(chalk.bold('  다음 단계:'));
    if (buildMode === 'unified') {
      console.log(
        chalk.dim('    1. ') + chalk.white('build-prompt.md') + chalk.dim(' 전체를 Claude에 붙여넣으세요.'),
      );
      console.log(
        chalk.dim('    2. 생성된 코드를 ') + chalk.white('.forge/generated/src/') + chalk.dim('에 저장하세요.'),
      );
    } else {
      console.log(
        chalk.dim('    1. ') + chalk.white('build-prompt.md') + chalk.dim('를 열면 블럭별 구분이 되어 있습니다.'),
      );
      console.log(
        chalk.dim('    2. ') + chalk.bold('--- BLOCK:') + chalk.dim(' 구분선 단위로 하나씩 Claude에 붙여넣으세요.'),
      );
      console.log(
        chalk.dim('    3. 생성된 코드를 블럭별로 ') +
          chalk.white('.forge/generated/src/') +
          chalk.dim('에 저장하세요.'),
      );
    }
    console.log();
...
function buildUnifiedPrompt(blocks, techStack, contracts, intent, phase) {
  ...
  const contractSummary = contracts.apis
    .map(
      (api) =>
