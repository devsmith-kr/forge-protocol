 ...
 */
export async function runShape(options = {}) {
  ...
  // 카탈로그를 로드해 concerns 등 전체 필드를 확보 (concerns 기반 프롬프트 조건화용)
  let fullBlocks = blocks;
  try {
    const catalog = await loadProjectCatalog(projectDir);
    const blockMap = buildBlockMap(catalog);
    fullBlocks = blocks.map((b) => ({ ...blockMap.get(b.id), ...b }));
  } catch {
    // 카탈로그 부재 시 legacy 휴리스틱으로 폴백 (concerns.js 에서 처리)
  }
  ...
  // 아키텍처 스타일 자동 선택 (team_size · 총 공수 · 블럭 수 기반 과설계 차단)
  const totalEffortDays = fullBlocks.reduce((s, b) => s + (b.effort_days ?? 0), 0);
  const archStyle = pickArchitectureStyle({
    blockCount: fullBlocks.length,
    serviceCount: features.length,
    totalEffortDays,
    teamSize: intent?.team_size ?? 1,
  });
  ...
  const architecture = {
    ...
    adr: buildADR(decisions, features, archStyle),
  };
  ...
  const prompt = buildArchitecturePrompt(intent, selectedBlocks, decisions, features, concerns);
  await writeFile(join(forgeDir, 'project', 'architecture-prompt.md'), prompt, 'utf-8');
  ...
  if (options.ai) {
    await runAiPrompt({
      promptText: prompt,
      outputPath: join(forgeDir, 'project', 'architecture-ai-response.md'),
      phaseName: 'Shape — 아키텍처 설계',
      model: options.model,
    });
    console.log(chalk.bold('  다음 단계:'));
    console.log(
      chalk.dim('    1. ') +
        chalk.white('architecture-ai-response.md') +
        chalk.dim(' 의 AI 설계 결과를 검토하세요.'),
    );
    console.log(chalk.dim('    2. ') + chalk.cyan('forge forge') + chalk.dim(' — API 계약 + 코드 생성 프롬프트'));
  } else {
    console.log(chalk.bold('  다음 단계:'));
    console.log(
      chalk.dim('    1. ') + chalk.white('architecture-prompt.md') + chalk.dim(' 내용을 Claude에 붙여넣으세요.'),
    );
    console.log(chalk.dim('    2. AI가 생성한 상세 아키텍처를 검토하세요.'));
    console.log(chalk.dim('    3. ') + chalk.cyan('forge forge') + chalk.dim(' — API 계약 + 코드 생성 프롬프트'));
    console.log();
...
function buildADR(decisions, features, archStyle) {
  const adrs = [];
  ...
  if (archStyle) {
    adrs.push({
      id: 'ADR-000',
      title: '아키텍처 스타일',
      decision: archStyle.choice,
      context: archStyle.reason,
      status: 'accepted',
      // 언제 바꿔야 하는가 — 전환 트리거 명시
      transition_triggers: archStyle.transition,
    });
  }
...
function buildArchitecturePrompt(intent, selectedBlocks, decisions, features, concerns) {
  const blockList = (selectedBlocks?.blocks ?? []).map((b) => `- ${b.name} (${b.effort_days}일)`).join('\n');

  const decisionList = Object.entries(decisions)
    .map(([k, v]) => `- ${k}: ${v}`)
    .join('\n');

  const featureList = features.map((f) => `- ${f.label}: ${f.note}`).join('\n');

  // concerns 기반 조건부 섹션 (결제/크롤링/인증/동시성 등 선택 블럭에 해당하는 경우에만)
  const concernBullets = buildConcernFragments(concerns, 'shape')
    .map((f) => `- ${f}`)
    .join('\n');
