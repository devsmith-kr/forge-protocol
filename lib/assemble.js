...
export async function runAssemble(options) {
  ...
  // ── 미매칭 항목 수동 처리 ──
  const allUnmatched = mapping.phases.flatMap(p =>
    p.unmatched.map(f => ({ phase: p.name, feature: f }))
  );
  ...
  const intent = {
    ...
    prerequisites: resolution.prerequisites.map(p => ({
      name: p.name, where: p.where, time: p.time, cost: p.cost,
    })),
  };
  ...
  const selectedBlocks = {
    blocks: resolution.allBlocks.map(id => {
      const block = blockMap.get(id);
      return { id, name: block?.name || id, priority: block?.priority || 'unknown', effort_days: block?.effort_days || 0 };
    }),
    total_effort_days: resolution.allBlocks.reduce((s, id) => s + (blockMap.get(id)?.effort_days || 0), 0),
  };
