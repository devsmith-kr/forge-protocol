...
function resolveAffected(blockId, deps) {
  return deps.filter((d) => d.source === blockId && d.type === 'affects').map((d) => d.target);
}
...
export function resolveAll(selectedBlockIds, catalog) {
  ...
  // 3. World 0 준비물 수집
  const neededPrereqs = prereqs.filter((p) => p.enables && p.enables.some((blockId) => requiredSet.has(blockId)));
