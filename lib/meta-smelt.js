...
async function runQuickMode(forgeDir, state) {
  ...
  const prompt = buildPrompt({ userMessageBody: buildQuickUserMessage(trimmed), modeLabel: 'Quick' });
...
async function runDeepMode(forgeDir, state) {
  ...
  const prompt = buildPrompt({ userMessageBody: buildDeepUserMessage(input), modeLabel: 'Deep' });
