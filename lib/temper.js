...
import { collectConcerns, buildConcernFragments, concernsOf } from '../shared/concerns.js';
import { buildScenariosForBlock } from '../shared/scenario-patterns.js';
import {
  loadState,
  loadArchitecture,
  loadContracts,
...
import { runAiPrompt } from './core/claude-client.js';
 ...
 */
export async function runTemper(options = {}) {
  ...
  if (focusMode === 'all') {
    ...
    console.log(
      chalk.dim(`  리스크 블럭 ${targetBlocks.length}개 선택됨: ${targetBlocks.map((b) => b.name).join(', ')}`),
    );
    console.log();
  } else {
  ...
  // temper-prompt.md — concerns 기반 조건부 섹션 포함
  const fullBlocks = targetBlocks.map((b) => ({ ...(blockMap.get(b.id) || {}), ...b }));
  const concerns = collectConcerns(fullBlocks);
  const prompt = buildTemperPrompt(scenarios, techStack, testTypes, contracts, concerns);
  await writeFile(join(forgeDir, 'project', 'temper-prompt.md'), prompt, 'utf-8');
  ...
  if (options.ai) {
    await runAiPrompt({
      promptText: prompt,
      outputPath: join(forgeDir, 'project', 'temper-ai-response.md'),
      phaseName: 'Temper — 테스트 코드 생성',
      model: options.model,
    });
    console.log(chalk.bold('  다음 단계:'));
    console.log(
      chalk.dim('    1. ') +
        chalk.white('temper-ai-response.md') +
        chalk.dim(' 의 테스트 코드를 검토하세요.'),
    );
    console.log(
      chalk.dim('    2. 테스트 코드를 ') + chalk.white('.forge/generated/test/') + chalk.dim('에 저장하세요.'),
    );
    console.log(
      chalk.dim('    3. ') + chalk.cyan('forge inspect') + chalk.dim(' — 보안/성능/운영/확장성 멀티 관점 리뷰'),
    );
  } else {
    console.log(chalk.bold('  다음 단계:'));
    console.log(
      chalk.dim('    1. ') + chalk.white('temper-prompt.md') + chalk.dim(' 내용을 Claude에 붙여넣으세요.'),
    );
    console.log(
      chalk.dim('    2. 생성된 테스트 코드를 ') + chalk.white('.forge/generated/test/') + chalk.dim('에 저장하세요.'),
    );
    console.log(
      chalk.dim('    3. ') + chalk.cyan('forge inspect') + chalk.dim(' — 보안/성능/운영/확장성 멀티 관점 리뷰'),
    );
    console.log();
    console.log(chalk.dim('  💡 --ai 플래그를 사용하면 Claude API로 자동 실행됩니다.'));
  }
  console.log();
}
...
function buildTestCases(block, catalogBlock, contract, detectedFeatures) {
  // P1-1: 패턴·concerns 기반 시나리오 생성 (shared/scenario-patterns.js)
  // catalogBlock 이 있으면 tech_desc/concerns 전체를 사용, 없으면 block 의 필드만.
  const merged = { ...(catalogBlock || {}), ...block };
  const concerns = concernsOf(merged);
  return buildScenariosForBlock(merged, { concerns });
}

/** @deprecated 과거 하드코딩 방식 — shared/scenario-patterns.js 로 이전됨 */
function legacyBuildTestCases(block, catalogBlock, contract, detectedFeatures) {
  const id = block.id ?? '';
  const cases = [];
...
function buildTemperPrompt(scenarios, techStack, testTypes, contracts, concerns) {
...
`
  ...
  const e2eGuide = testTypes.includes('e2e')
    ? `
#### E2E 테스트 규칙
- 핵심 사용자 시나리오의 golden path 흐름만 다룹니다.
- TestRestTemplate 또는 RestAssured 로 실제 HTTP 요청.
- 외부 시스템은 Mock 서버(WireMock/MockWebServer)로 대체.
`
    : '';

  // concerns 기반 조건부 추가 요청사항
  const concernBullets = buildConcernFragments(concerns, 'temper')
    .map((f) => `- ${f}`)
    .join('\n');
