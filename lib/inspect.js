...
import ora from 'ora';
import { loadState, loadArchitecture, loadContracts, loadTestScenarios, loadYaml, saveState } from './core/project.js';
...
export async function runInspect(options = {}) {
  ...
  const report = buildForgeReport(
    ...
    contracts,
    selectedFindings,
    checklistByPerspective,
    perspectives,
    reportLevel,
    testScenarios,
  );
  await writeFile(join(forgeDir, 'project', 'forge-report.md'), report, 'utf-8');
  ...
  await writeFile(join(forgeDir, 'project', 'inspect-prompt.md'), inspectPrompt, 'utf-8');
  ...
  if (options.ai) {
    await runAiPrompt({
      promptText: inspectPrompt,
      outputPath: join(forgeDir, 'project', 'inspect-ai-response.md'),
      phaseName: 'Inspect — 상세 리뷰',
      model: options.model,
    });
    console.log(chalk.bold('  다음 단계:'));
    console.log(
      chalk.dim('    1. ') + chalk.white('forge-report.md') + chalk.dim(' 를 직접 검토하세요.'),
    );
    console.log(
      chalk.dim('    2. ') +
        chalk.white('inspect-ai-response.md') +
        chalk.dim(' 의 AI 상세 개선안을 검토하세요.'),
    );
    ...
    console.log(
      chalk.dim('    2. ') +
        chalk.white('inspect-prompt.md') +
        chalk.dim(' 를 Claude에 붙여넣으면 상세 개선안이 제시됩니다.'),
    );
    console.log(chalk.dim('    3. 위험 항목 해결 후 코드 구현을 시작하세요.'));
    console.log();
...
function buildChecklist(blocks, techStack, detectedFeatures, perspectives) {
  ...
  if (perspectives.includes('scalability')) {
    result.scalability = [
      ...
      {
        item: '분산 락 전략 — Optimistic vs Pessimistic vs Redis 기준 정의',
        done: detectedFeatures.has('concurrency') ? false : null,
      },
    ].filter((c) => c.done !== null);
  }
...
function buildForgeReport(
  ...
  const findingsSection = perspectives
    .map((p) => {
  ...
  const checklistSection = Object.entries(checklist)
    .map(([p, items]) => {
      const label = { security: '🔐 보안', performance: '⚡ 성능', operations: '🔧 운영', scalability: '📈 확장성' }[p];
...
function buildInspectPrompt(
  ...
  const perspectiveInstructions = perspectives
    .map((p) => {
      const instructions = {
        security: `#### 보안 리뷰 요청
...
${(() => {
  const bullets = buildConcernFragments(concerns, 'inspect')
    .map((f) => `- ${f}`)
    .join('\n');
