...
describe('buildScenariosForBlock', () => {
  ...
  it('crawler 블럭 → robots.txt / 429 백오프 / 파싱 실패율 시나리오', () => {
    const s = buildScenariosForBlock({ id: 'source-crawler', name: '외부 사이트 크롤러', tech_desc: 'robots.txt 준수, 지수 백오프' });
    const names = s.map(c => c.name).join(' ');
  ...
  it('dedupe/SimHash 블럭 → hamming 경계·tie-break 시나리오', () => {
    const s = buildScenariosForBlock({
      id: 'duplicate-detector', name: '중복 공고 제거',
  ...
  it('search 블럭 → 빈쿼리·XSS·Nori 시나리오', () => {
    const s = buildScenariosForBlock({ id: 'job-search', name: '통합 공고 검색', tech_desc: 'Elasticsearch 기반 Full-text 검색' });
  ...
  it('시나리오 수가 6개로 제한됨 (과다 생성 방지)', () => {
    ...
    const s = buildScenariosForBlock({
      id: 'mega-auth-payment-notification', name: '합성',
